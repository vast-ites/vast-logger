...
const PostgreSQLDetail = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await fetch(`/api/v1/services/postgresql/db-stats`, { headers });
            if (res.ok) {
            ...
            {!stats && (
                <div className="bg-cyber-yellow/10 border border-cyber-yellow/30 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-cyber-muted text-sm mt-1">PostgreSQL monitoring requires the agent to have access on port 5432.</p>
                    </div>
                </div>
            )}
