...
const MySQLDetail = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await fetch(`/api/v1/services/mysql/db-stats?host=${selectedHost || ''}`, { headers });
            if (res.ok) {
    ...
    const hasData = stats && !isStale && (stats.threads_connected > 0 || stats.queries_per_second > 0);
    const s = stats || {};
